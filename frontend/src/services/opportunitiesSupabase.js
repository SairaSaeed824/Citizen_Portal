// Legacy filename retained for compatibility. All opportunity data access goes through FastAPI.
...
export async function getOpportunitiesSupabase(filters = {}) {
  const { category = 'all', province = 'all', location = '', organization = '', deadline = 'all', keyword = '', status = 'all', sortBy = 'default', page = 1, limit = 50, returnMeta = false } = filters;
  ...
  let results = (result.data || []).map((item) => {
    ...
    return { ...item, id: item.id, category: item.category || extra.category || category || 'job', title: item.title || extra.title || extra.job_title || extra.name || 'Untitled Opportunity', organization: item.organization || extra.organization || extra.company || extra.department || extra.ministry || '', description: item.description || extra.description || extra.details || extra.program_overview || extra.overview || extra.job_description || '', province: getProvinceValue(item), location: item.location || extra.location || extra.city || extra.district || '', closing_date: item.closing_date || extra.closing_date || extra.deadline || extra.last_date || '', status: item.status || extra.status || '', url: item.url || extra.url || extra.link || extra.apply_url || '', extra_data: extra };
  });
  ...
  if (returnMeta) return { data: results, total: result.total ?? results.length, page: result.page ?? page, limit: result.limit ?? limit, hasNext: Boolean(result.has_next) };
  return results;
}

export async function getOpportunityByIdSupabase(id) {
  const response = await fetch(`${API_BASE_URL}/api/opportunities/${encodeURIComponent(Number(id))}`);
...
export async function getProvincesSupabase() {
  ...
  try {
    ...
    if (response.ok) values = ((await response.json()).data || []).map(getProvinceValue).filter(Boolean);
  } catch (error) {
