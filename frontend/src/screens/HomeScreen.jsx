...
import { Search, RotateCcw, FolderOpen, Heart, Trash2, Clock, MapPin, CalendarDays, Sparkles } from 'lucide-react';
      ...
      <section id="directory" className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="mb-7 max-w-5xl mx-auto rounded-2xl border border-emerald-100 bg-white p-4 sm:p-5 shadow-sm shadow-emerald-100/50 dark:border-emerald-900/50 dark:bg-slate-900 dark:shadow-none">
          <div className="text-center mb-4">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Search by title and refine results using province, location, deadline, and recommendations.</p>
          </div>
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <Select value={selectedProvince} onValueChange={setSelectedProvince}>
              <SelectTrigger aria-label="Filter by province"><div className="flex items-center gap-2 min-w-0"><MapPin className="h-4 w-4 text-emerald-700 dark:text-emerald-400 shrink-0" /><SelectValue placeholder="All Provinces / Regions" /></div></SelectTrigger>
              <SelectContent>
            ...
            <div className="relative">
              ...
              <Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="City / location" className="pl-9" aria-label="Filter by city or location" />
            </div>

            <Select value={sortBy} onValueChange={setSortBy}>
              <SelectTrigger aria-label="Recommended opportunities"><div className="flex items-center gap-2 min-w-0"><Sparkles className="h-4 w-4 text-amber-500 shrink-0" /><SelectValue placeholder="Recommended" /></div></SelectTrigger>
              <SelectContent>
            ...
            <Select value={deadline} onValueChange={setDeadline}>
              <SelectTrigger aria-label="Filter by deadline"><div className="flex items-center gap-2 min-w-0"><CalendarDays className="h-4 w-4 text-slate-400 shrink-0" /><SelectValue placeholder="Any Deadline" /></div></SelectTrigger>
              <SelectContent>
          ...
          <div className="mt-3 flex justify-center">
            <Button type="button" variant="outline" onClick={clearFilters} disabled={!activeFilters} className="gap-2 h-10">
